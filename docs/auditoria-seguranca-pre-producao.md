...
- Variáveis no checkpoint 7D: `DATABASE_URL`, `TEST_DATABASE_URL`, `NODE_ENV`, `FRONTEND_ORIGIN`, `SESSION_DURATION_HOURS`, `COOKIE_SECURE`, `CSRF_SECRET`, `HOST`, `PORT`, `TZ` e limites de importação. A Etapa 8A substituiu `VITE_API_URL` por `VITE_DEV_API_ORIGIN`; chamadas do frontend continuam em `/api`, sem credenciais embutidas.
...
- No checkpoint 7D, `trustProxy:false` não confiava em X-Forwarded-For e os timeouts observados eram request/connection 0 e keep-alive 72s. A Etapa 8A manteve o proxy fechado, definiu timeouts do servidor e documentou a validação obrigatória da borda; limites em memória ainda não garantem resistência a botnet, reinício, múltiplas instâncias ou DoS de borda.
...
O checkpoint foi registrado no commit local `a56577e` (`security: conclui auditoria final pre-producao`) após autorização. `frontend/index.html` permaneceu fora desse commit e nenhum `git push` foi executado.
